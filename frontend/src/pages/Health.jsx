...
const Health = () => {
  const { currentData, selectedLocation, userGroup, setUserGroup } = useAirQualityContext();
  ...
  const getHealthImpact = (aqi) => {
    if (aqi <= 50) return { level: 'minimal', color: '#10B981', description: 'Air quality is satisfactory for most people' };
    if (aqi <= 100) return { level: 'low', color: '#F59E0B', description: 'Acceptable for most, sensitive individuals may experience minor issues' };
    if (aqi <= 200) return { level: 'moderate', color: '#EF4444', description: 'Unhealthy for sensitive groups, may cause symptoms' };
    if (aqi <= 300) return { level: 'high', color: '#DC2626', description: 'Everyone may experience health effects' };
    return { level: 'severe', color: '#7F1D1D', description: 'Health alert: serious health effects for everyone' };
  };
  ...
  const getHealthTips = (userGroup, aqi) => {
    ...
    return baseTips.length > 0 ? baseTips : ['Air quality is generally safe for outdoor activities'];
  };
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-pink-100">
              Personalized health guidance based on air quality in {selectedLocation}
            </p>
          </div>
          <div className="text-right">
            ...
            <div className="text-2xl font-bold capitalize">{healthImpact.level}</div>
            <div className="text-sm text-pink-100">AQI: {Math.round(currentAQI)}</div>
          </div>
        </div>
      </div>
      ...
      <div className="aqi-card">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-700">Select Your Profile</h3>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { id: 'general', label: 'General Public', icon: '👥', desc: 'Healthy adults' },
            { id: 'sensitive', label: 'Sensitive Groups', icon: '🫁', desc: 'Asthma, COPD, heart disease' },
            { id: 'elderly', label: 'Elderly (65+)', icon: '👴', desc: 'Senior citizens' },
            { id: 'children', label: 'Children', icon: '👶', desc: 'Under 18 years' }
          ].map((group) => (
            <button
              key={group.id}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="aqi-card">
          <div className="flex items-center space-x-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-gray-700">Current Risk Level</h3>
          </div>
          ...
          <div className="text-center mb-4">
            ...
            <div className="text-sm text-gray-600">{healthImpact.description}</div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium capitalize">{userGroup.replace('_', ' ')}</span>
            </div>
          </div>
        </div>
        ...
        <div className="aqi-card">
          <div className="flex items-center space-x-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-gray-700">Potential Symptoms</h3>
          </div>
          ...
          <div className="space-y-3">
            {currentAQI <= 50 && (
              <div className="flex items-center space-x-3">
                ...
                <span className="text-sm text-gray-700">No symptoms expected</span>
              </div>
            )}
            ...
            {currentAQI > 50 && currentAQI <= 100 && (
              <>
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-sm text-gray-700">Mild respiratory irritation (sensitive individuals)</span>
                </div>
              </>
            )}
            ...
            {currentAQI > 100 && currentAQI <= 200 && (
              <>
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-sm text-gray-700">Coughing, throat irritation</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-orange-500 rounded-full"></div>
                  <span className="text-sm text-gray-700">Shortness of breath during exercise</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-orange-500 rounded-full"></div>
                  <span className="text-sm text-gray-700">Eye and nose irritation</span>
                </div>
              </>
            )}
            ...
            {currentAQI > 200 && (
              <>
                ...
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-red-500 rounded-full"></div>
                  <span className="text-sm text-gray-700">Increased heart rate</span>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-red-500 rounded-full"></div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="aqi-card">
            <div className="flex items-center space-x-2 mb-4">
              ...
              <h3 className="text-lg font-semibold text-gray-700">Pollutant Health Effects</h3>
            </div>

            <div className="space-y-4">
              {pollutantData.pollutant_breakdown && Object.entries(pollutantData.pollutant_breakdown).map(([pollutant, data]) => (
                <div key={pollutant} className="p-3 border border-gray-200 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600 mb-2">{data.health_impact}</p>
                  <div className="text-xs text-gray-500">
                    Current: {data.value} {data.unit} | Category: {data.category}
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h4 className="font-semibold text-red-900">When to Seek Medical Attention</h4>
            ...
            <div className="mt-3 text-xs text-red-600">
              <strong>Emergency:</strong> Call emergency services if experiencing severe breathing difficulties or chest pain.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h4 className="font-semibold text-blue-900">Health Guidance Sources</h4>
            <p className="text-blue-700 text-sm mt-1">
              Health recommendations are based on guidelines from WHO, EPA, and Indian Central Pollution Control Board (CPCB). 
              Individual health responses may vary. Consult healthcare professionals for personalized medical advice.
            </p>
          </div>
        </div>
      </div>
